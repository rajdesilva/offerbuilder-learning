import { isSavedOfferProperty } from "../isSavedOfferProperty";

describe("isSavedOfferProperty function test", () => {
  test("isSavedOfferProperty check if current property is saved", () => {
    const property = {
      img:
    ...
    expect(isSavedOfferProperty(property, selectedProperties)).toBe(true);
  });
  test("isSavedOfferProperty check if property is not saved for offer", () => {
    const property = {
      img:
        "https://res.cloudinary.com/seekda-dev/image/upload/w_1920,h_1080,c_fill,f_auto,fl_lossy,q_auto/development/DEMO_JEEVAN/chambre_double_green_fullsize_ecodef-1140x760.jpg",
      images: [
    ...
    expect(isSavedOfferProperty(property, selectedProperties)).toBe(false);
  });
});
