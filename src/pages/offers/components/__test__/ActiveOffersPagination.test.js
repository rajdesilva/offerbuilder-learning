...
describe("ActiveOffersPagination test", () => {
  test("create ActiveOffersPagination components snapshot", () => {
  ...
  test("check pagination is available or not", async () => {
    ...
    expect(getByTestId("active-offer-list-pagination")).toBeInTheDocument();
  ...
  test("check pagination for 20 per page available or not", async () => {
    ...
    const searchAndFilterOfferFn = jest.spyOn(service, "searchAndFilterOffer");
    const onPageChange = getByTestId("active-offer-list-pagination").querySelector("input");
    await act(async () => {
  ...
  test("ActiveOffersPagination check if pagination available or not for total offers 0", async () => {
    ...
    expect(queryByTestId("active-offer-list-pagination")).not.toBeInTheDocument();
